...
regular_access = AccountType.objects.get(type_id='PS-HPA')

applications = Application.objects.filter(account_type=regular_access)

print("pk | pi_time | username | name |  id_no | department | cpu_core_hours | gpu_node_hours | amount | domain | sub_domain | software | contact | gender")

for a in applications:
    if not a.hpc_profile.pi_profile and a.pi_time is not None:
